...
from vllm_ascend.worker.npu_cache_manager import NPUCacheManager
if TYPE_CHECKING:
...
class NPUModelRunner:
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        self.num_q_heads = self.model_config.get_num_attention_heads(vllm_config.parallel_config)
        ...
        self.input_batch = InputBatch(
            ...
            block_repr=torch.zeros((self.num_kv_heads, 1, self.head_size), dtype=self.kv_cache_dtype, device=self.device),
        )
    ...
    def init_npu_cache_manager(self, num_gpu_cache_blocks: int, num_cpu_blocks: int, block_size: int, sparse_topk: Optional[int] = None,
                               copy_method: str = "raw",
                               cache_policy: str = "lru"):
        assert self.npu_cache_manager is None
        self.npu_cache_manager = NPUCacheManager(
            ...
            block_repr_table=self.input_batch.block_table.get_block_repr_table(),
    ...
    def _process_reqs(
        ...
        self.query_lens_np = num_scheduled_tokens
        ...
        # Run forward pass
        with set_forward_context(attn_metadata, self.vllm_config, gpu_cache_manager=self.npu_cache_manager):
    ...
    def initialize_kv_cache(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        for kv_cache_group in kv_cache_config.kv_cache_groups:
            ...
            for layer_name in kv_cache_group.layer_names:
                ...
                if isinstance(kv_cache_spec, FullAttentionSpec):
                    if npu_kv_cache is None:
                        kv_cache_shape = self.attn_backend.get_kv_cache_shape(
                            num_gpu_blocks, kv_cache_spec.block_size,
                            kv_cache_spec.num_kv_heads, kv_cache_spec.head_size)
                        dtype = kv_cache_spec.dtype
                        npu_kv_cache = torch.zeros(kv_cache_shape,
                                                    dtype=dtype,
                                                    device=self.device)
                        torch_npu.npu_format_cast(npu_kv_cache, 2)
                    kv_caches[layer_name] = npu_kv_cache
        ...
        # 检查 NPU/CPU KV Cache 是否绑定成功
        assert all(npu_kv_cache.data_ptr() == kv_.data_ptr() for kv_ in self.kv_caches)
        assert all(npu_kv_cache.data_ptr() == s.kv_cache[0].data_ptr() for s in self.vllm_config.compilation_config.static_forward_context.values())
